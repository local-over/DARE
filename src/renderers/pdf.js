...
async function renderPdf(astData, outputPath) {
    ...
    const docDefinition = {
        ...
        header: headerNode ? function(currentPage, pageCount) { 
            return { ...headerNode, margin: [defaultMargin, 10, defaultMargin, 0] }; 
        } : undefined,
        footer: footerNode ? function(currentPage, pageCount) { 
            ...
            return { ...footerCopy, margin: [defaultMargin, 0, defaultMargin, 10] }; 
        } : undefined,
        defaultStyle: {
            font: 'Roboto',
            lineHeight: 1.4
    ...
    if (outputPath) {
        ...
        return new Promise((resolve, reject) => {
            ...
            pdfStream.on('end', () => resolve(outputPath));
            pdfStream.on('error', reject);
            pdfStream.end();
        });
    } else {
        // Return Uint8Array for Edge / API
        return new Promise((resolve, reject) => {
